...
module.exports = function(locals, type, path) {
  ...
  let posts = locals.data.breaches.breaches.filter((item) => !item.Name || !item.Name.toLowerCase().includes('cve')).map((
    item,index
  )=>{
  ...
  posts=posts.sort((a, b) => new Date(b.BreachDate) - new Date(a.BreachDate)).filter((item)=>{
          console.log(item.IsRetired)
          return !item.IsRetired
        });
let up =locals.data.breaches.lastUpdated
