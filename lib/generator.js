...
function composeItem(post, feedConfig, context) {
  ...
  const updated = (post.ModifiedDate && typeof post.ModifiedDate.toDate === 'function') ? post.ModifiedDate.toDate() : new Date(post.ModifiedDate || post.date || Date.now());
...
module.exports = function(locals, type, path) {
  ...
  // Override feedUrl with the one generated in this context to ensure consistency
  feed.feedUrl = feed_url;
  
  const items = posts.map((post, index) => {
    return composeItem(post, feedConfig, this);
  });

  // Ensure uniqueness of updated date for entries sharing the same timestamp in Atom feeds.
  // This resolves validation errors like "Two entries with the same value for atom:updated"
  if (type !== 'rss2') {
    const seenUpdates = new Set();
    items.forEach((item) => {
      if (item.updated) {
        let timestamp = item.updated.getTime();
        if (seenUpdates.has(timestamp)) {
          // Find a unique timestamp by subtracting 1ms at a time
          while (seenUpdates.has(timestamp)) {
            timestamp -= 1;
          }
          item.updated = new Date(timestamp);
        }
        seenUpdates.add(timestamp);
      }
    });
  }
