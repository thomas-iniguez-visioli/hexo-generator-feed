...
module.exports = function(locals, type, path) {
  ...
  let posts = locals.data.breaches.breaches.map((
    item,index
  )=>{
    item.permalink="https://feed-blush.vercel.app/fuites?id="+index
    if(item.date){
      return item
    }else{
  ...
  posts=posts.sort((a, b) => new Date(b.BreachDate) - new Date(a.BreachDate));
